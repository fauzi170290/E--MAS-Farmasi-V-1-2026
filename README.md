...
Jika file source tidak tersedia pada komputer DBA, blok berikut dapat langsung
di-copy-paste ke phpMyAdmin, HeidiSQL, atau MariaDB client setelah database
target yang benar dipilih. Untuk database produksi Khanza gunakan `USE sik;`;
untuk clone uji gunakan `USE sik_emss_uji_lokal;`.
...
SELECT
    ...
    CASE
        ...
        THEN SHA2(CONCAT_WS('|', CONVERT(ro.no_resep USING utf8mb4),
            CAST(ro.tgl_perawatan AS CHAR), CAST(ro.jam AS CHAR),
            CAST(ro.tgl_penyerahan AS CHAR), CAST(ro.jam_penyerahan AS CHAR)), 256)
        ELSE ''
    ...
    CASE
        WHEN NULLIF(CAST(ro.tgl_penyerahan AS CHAR), '0000-00-00') IS NOT NULL
            THEN STR_TO_DATE(CONCAT(CAST(ro.tgl_penyerahan AS CHAR), ' ',
                CAST(ro.jam_penyerahan AS CHAR)), '%Y-%m-%d %H:%i:%s')
        WHEN NULLIF(CAST(ro.tgl_perawatan AS CHAR), '0000-00-00') IS NOT NULL
            THEN STR_TO_DATE(CONCAT(CAST(ro.tgl_perawatan AS CHAR), ' ',
                CAST(ro.jam AS CHAR)), '%Y-%m-%d %H:%i:%s')
        ELSE STR_TO_DATE(CONCAT(CAST(ro.tgl_peresepan AS CHAR), ' ',
            CAST(ro.jam_peresepan AS CHAR)), '%Y-%m-%d %H:%i:%s')
    END AS changed_at
...
GROUP BY rd.no_resep, rd.kode_brng,
    COALESCE(NULLIF(db.nama_brng, ''), rd.kode_brng), rd.aturan_pakai;
...
LEFT JOIN resep_dokter_racikan AS rr
    ON rr.no_resep = rdd.no_resep AND rr.no_racik = rdd.no_racik
